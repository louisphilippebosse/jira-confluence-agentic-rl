"""
Agent & Skill Registry API - CRUD operations for agents and skills
"""
    ...
    try:
        ...
        if agent.skill_ids:
            for skill_id in agent.skill_ids:
                ...
                db.add(AgentSkill(agent_id=db_agent.id, skill_id=skill_id))
...
async def add_skill_to_agent(agent_id: int, skill_add: AgentSkillAdd, db: Session = Depends(get_db)):
...
async def remove_skill_from_agent(agent_id: int, skill_id: int, db: Session = Depends(get_db)):
